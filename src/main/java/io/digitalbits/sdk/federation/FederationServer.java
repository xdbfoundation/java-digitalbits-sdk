...
import okhttp3.*;
import io.digitalbits.sdk.requests.ResponseHandler;
 ...
 * <a href="https://developers.digitalbits.io/guides/concepts/federation.html" target="_blank">federation server</a>
 * instance and exposes an interface for requests to that instance.
 *
 * For resolving a DigitalBits address without knowing which federation server
...
public class FederationServer {
   ...
   * It tries to find a federation server URL in digitalbits.toml file.
   * @see <a href="https://developers.digitalbits.io/guides/concepts/digitalbits-toml.html" target="_blank">digitalbits.toml docs</a>
  ...
  public static FederationServer createForDomain(String domain) {
    ...
    uriBuilder.append("/.well-known/digitalbits.toml");
    HttpUrl digitalbitsTomlUri = HttpUrl.parse(uriBuilder.toString());
    OkHttpClient httpClient = FederationServer.createHttpClient();

    Request request = new Request.Builder().get().url(digitalbitsTomlUri).build();
    Response response = null;
    try {
      response = httpClient.newCall(request).execute();

      if (response.code() >= 300) {
        throw new DigitalBitsTomlNotFoundInvalidException();
      }

      Toml digitalbitsToml = new Toml().read(response.body().string());

      String federationServer = digitalbitsToml.getString("FEDERATION_SERVER");
   ...
   * @param address DigitalBits addres, like <code>bob*digitalbits.io</code>
