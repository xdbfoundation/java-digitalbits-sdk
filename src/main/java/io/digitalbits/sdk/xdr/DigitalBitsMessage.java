...
//  ===========================================================================
public class DigitalBitsMessage implements XdrElement {
  public DigitalBitsMessage () {}
  MessageType type;
  ...
  public static final class Builder {
    ...
    public DigitalBitsMessage build() {
      DigitalBitsMessage val = new DigitalBitsMessage();
  ...
  public static void encode(XdrDataOutputStream stream, DigitalBitsMessage encodedDigitalBitsMessage) throws IOException {
  //Xdrgen::AST::Identifier
  //MessageType
  stream.writeInt(encodedDigitalBitsMessage.getDiscriminant().getValue());
  switch (encodedDigitalBitsMessage.getDiscriminant()) {
  case ERROR_MSG:
  Error.encode(stream, encodedDigitalBitsMessage.error);
  break;
  case HELLO:
  Hello.encode(stream, encodedDigitalBitsMessage.hello);
  break;
  case AUTH:
  Auth.encode(stream, encodedDigitalBitsMessage.auth);
  break;
  case DONT_HAVE:
  DontHave.encode(stream, encodedDigitalBitsMessage.dontHave);
  break;
  case GET_PEERS:
  break;
  case PEERS:
  int peerssize = encodedDigitalBitsMessage.getPeers().length;
  stream.writeInt(peerssize);
  for (int i = 0; i < peerssize; i++) {
    PeerAddress.encode(stream, encodedDigitalBitsMessage.peers[i]);
  }
  break;
  case GET_TX_SET:
  Uint256.encode(stream, encodedDigitalBitsMessage.txSetHash);
  break;
  case TX_SET:
  TransactionSet.encode(stream, encodedDigitalBitsMessage.txSet);
  break;
  case TRANSACTION:
  TransactionEnvelope.encode(stream, encodedDigitalBitsMessage.transaction);
  break;
  case SURVEY_REQUEST:
  SignedSurveyRequestMessage.encode(stream, encodedDigitalBitsMessage.signedSurveyRequestMessage);
  break;
  case SURVEY_RESPONSE:
  SignedSurveyResponseMessage.encode(stream, encodedDigitalBitsMessage.signedSurveyResponseMessage);
  break;
  case GET_SCP_QUORUMSET:
  Uint256.encode(stream, encodedDigitalBitsMessage.qSetHash);
  break;
  case SCP_QUORUMSET:
  SCPQuorumSet.encode(stream, encodedDigitalBitsMessage.qSet);
  break;
  case SCP_MESSAGE:
  SCPEnvelope.encode(stream, encodedDigitalBitsMessage.envelope);
  break;
  case GET_SCP_STATE:
  Uint32.encode(stream, encodedDigitalBitsMessage.getSCPLedgerSeq);
  break;
  case SEND_MORE:
  SendMore.encode(stream, encodedDigitalBitsMessage.sendMoreMessage);
  break;
  }
  }
  ...
  public static DigitalBitsMessage decode(XdrDataInputStream stream) throws IOException {
  DigitalBitsMessage decodedDigitalBitsMessage = new DigitalBitsMessage();
  MessageType discriminant = MessageType.decode(stream);
  decodedDigitalBitsMessage.setDiscriminant(discriminant);
  switch (decodedDigitalBitsMessage.getDiscriminant()) {
  case ERROR_MSG:
  decodedDigitalBitsMessage.error = Error.decode(stream);
  break;
  case HELLO:
  decodedDigitalBitsMessage.hello = Hello.decode(stream);
  break;
  case AUTH:
  decodedDigitalBitsMessage.auth = Auth.decode(stream);
  break;
  case DONT_HAVE:
  decodedDigitalBitsMessage.dontHave = DontHave.decode(stream);
  break;
  case GET_PEERS:
  break;
  case PEERS:
  int peerssize = stream.readInt();
  decodedDigitalBitsMessage.peers = new PeerAddress[peerssize];
  for (int i = 0; i < peerssize; i++) {
    decodedDigitalBitsMessage.peers[i] = PeerAddress.decode(stream);
  }
  break;
  case GET_TX_SET:
  decodedDigitalBitsMessage.txSetHash = Uint256.decode(stream);
  break;
  case TX_SET:
  decodedDigitalBitsMessage.txSet = TransactionSet.decode(stream);
  break;
  case TRANSACTION:
  decodedDigitalBitsMessage.transaction = TransactionEnvelope.decode(stream);
  break;
  case SURVEY_REQUEST:
  decodedDigitalBitsMessage.signedSurveyRequestMessage = SignedSurveyRequestMessage.decode(stream);
  break;
  case SURVEY_RESPONSE:
  decodedDigitalBitsMessage.signedSurveyResponseMessage = SignedSurveyResponseMessage.decode(stream);
  break;
  case GET_SCP_QUORUMSET:
  decodedDigitalBitsMessage.qSetHash = Uint256.decode(stream);
  break;
  case SCP_QUORUMSET:
  decodedDigitalBitsMessage.qSet = SCPQuorumSet.decode(stream);
  break;
  case SCP_MESSAGE:
  decodedDigitalBitsMessage.envelope = SCPEnvelope.decode(stream);
  break;
  case GET_SCP_STATE:
  decodedDigitalBitsMessage.getSCPLedgerSeq = Uint32.decode(stream);
  break;
  case SEND_MORE:
  decodedDigitalBitsMessage.sendMoreMessage = SendMore.decode(stream);
  break;
  }
    return decodedDigitalBitsMessage;
  }
  @Override
  public int hashCode() {
  ...
  public boolean equals(Object object) {
    if (!(object instanceof DigitalBitsMessage)) {
      return false;
    }

    DigitalBitsMessage other = (DigitalBitsMessage) object;
