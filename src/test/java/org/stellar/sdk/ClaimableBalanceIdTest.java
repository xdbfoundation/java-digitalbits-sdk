...
public class ClaimableBalanceIdTest {
  ...
  public void testClaimableBalanceIds() throws IOException {
    ...
    transaction = new Transaction.Builder(AccountConverter.enableMuxed(), new Account(StrKey.encodeDigitalBitsMuxedAccount(muxedAccount), 123l), Network.TESTNET)
        .addOperation(op0)
        .addOperation(new BumpSequenceOperation.Builder(2l).build())
        .setTimeout(Transaction.Builder.TIMEOUT_INFINITE)
        .setBaseFee(Transaction.MIN_BASE_FEE)
    ...
    try {
      ...
      assertEquals("operation at index 1 is not of type CreateClaimableBalanceOperation: class io.digitalbits.sdk.BumpSequenceOperation", e.getMessage());
    }
  }
}
